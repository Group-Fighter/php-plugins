class ExportManager {
	...
	async sendAjaxRequest(url, data) {
		try {
			...
			this.downloadXLSX(responseData.data.link, responseData.data.file, responseData.data.delete);
		} catch (error) {
			this.alertError('Terjadi kesalahan saat mengunduh data.');
		}
	}

	async downloadXLSX(url, fileName = 'downloadedFile.xlsx', afterDownloadUrl = '') {
		try {
			const response = await fetch(url);
	...
	alertError(message) {
		// Ganti dengan implementasi notifikasi error (misalnya menggunakan Toast atau alert biasa)
		console.error(message);
		alert(message);
	}

	alertSuccess(message) {
		// Ganti dengan implementasi notifikasi sukses
		console.log(message);
		alert(message);
	}
}
